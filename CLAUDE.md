...
Two state-changing items, both context-aware (label + action swap on `_connected` in `_setUiState`) and both arming the fast poll (via `_spawnPrivileged`) so the socket-poll catches the result quickly:
...
**Why pkexec, not `twingate start`/`stop` directly:** the daemon (`twingate.service`) is root-owned, and `twingate start`/`stop`/`connect`/`disconnect` all shell out to `sudo twingate service-start`/`service-stop` internally. Spawned from GNOME Shell there is no controlling terminal, so that sudo aborts (`sudo: a terminal is required to read the password`) — the earlier fire-and-forget `GLib.spawn_command_line_async('twingate start')` did nothing and tripped apport ("Ubuntu has encountered an issue"). `_spawnPrivileged(privilegedArgv, followUps=[])` instead runs `pkexec twingate service-…` (which pops the desktop's polkit admin-password modal via gnome-shell's polkit agent), tracks the proc in `this._procs`, waits with `wait_async` on the shared `this._cancellable`, and only on `get_successful()` arms the fast poll and fires the user-level follow-ups. The follow-ups need no sudo because the daemon is already in the target state (verified: `twingate start` with the daemon up, and user-level `connect`/`desktop-*`, never invoke sudo). Cancelling the password dialog leaves state untouched. **The only privileged operation is the systemd daemon start/stop** — everything else (connect handshake, tokens, desktop notifier) is user-level.

Everything that needs output uses `Gio.Subprocess` instead, via two shared helpers:
- `_runForLines(argv, cb)` — runs a command, hands stdout (or `null` on failure) to `cb`. Every spawned proc is added to `this._procs` and removed on completion so `stop()` can `force_exit` anything still in flight. Both this and `_loadResources` pass the shared `this._cancellable` to `communicate_utf8_async` and bail at the top of the callback if it's cancelled — `force_exit` does **not** cancel the already-queued async finish callback, so without this a late callback could touch the disposed GObject after `disable()`. `stop()` calls `this._cancellable.cancel()` right after stopping the poll source.
...
Service and Notifications are **submenus** (`_serviceMenu` / `_notifMenu`, built by `_buildCommandSubMenu(title, entries)`) of fixed one-shot commands: Service → `service-start`/`service-stop` (no `service-restart` exists in the CLI); Notifications → `desktop-start`/`desktop-stop`/`desktop-restart`. Entries flagged `privileged: true` (the root-owned Service commands) are routed through `_spawnPrivileged` (pkexec) instead of `GLib.spawn_command_line_async`, for the same reason as the toggle — their static `command` string is split into argv (safe, no user data). Notifications are user-level and stay on the plain spawn path. Each child item's `activate` handle is tracked in `this._subItemHandles` and disconnected in `stop()` (the submenu `destroy()` tears down the child actors).
